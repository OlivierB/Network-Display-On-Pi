...
*/
  ...
  function BuildUniqueAddressLinkPerso($addr_type, $raw = "" )
  {
     return '<A HREF="/base/base_stat_uaddr.php?addr_type='.$addr_type.$raw.'">';
  }
  ...
  while ( ($myrow = $result->baseFetchRow()) && ($i < $qs->GetDisplayRowCnt()) )
  {
     ...
     qroPrintEntry('<FONT>'.
                   '<A HREF="/base/base_qry_main.php?new=1amp;&amp;sig%5B0%5D=%3D&amp;sig%5B1%5D='.
     ...
     qroPrintEntry('<A HREF="/base/base_stat_sensor.php?sig%5B0%5D=%3D&amp;sig%5B1%5D='.
     ...
     qroPrintEntry('<FONT>'.BuildUniqueAddressLinkPerso(1, $addr_link).$num_src_ip.'</A></FONT>');
     qroPrintEntry('<FONT>'.BuildUniqueAddressLinkPerso(2, $addr_link).$num_dst_ip.'</A></FONT>');

     if ( $show_first_last_links == 1 ) {
       qroPrintEntry('<FONT>'.
	             '<A HREF="/base/base_qry_alert.php?'.
       ...
       $date = new DateTime($stop_time);
       $date->add(new DateInterval('PT10M'));
       
        if(new DateTime('now') < $date)
          qroPrintEntry('<FONT class="stop-time recent">'.$stop_time.'</FONT>');
        else
          qroPrintEntry('<FONT class="stop-time">'.$stop_time.'</FONT>');
